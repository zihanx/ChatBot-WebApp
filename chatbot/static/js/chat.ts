...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    for (const chatText of chatTexts) {
        if (chatText.textContent) {
            const markdownText = chatText.textContent.trim(); // Ensure no null values
            chatText.innerHTML = await marked.parse(markdownText); // Await for proper parsing
        }
    }
    ...
    async function streamAIResponse(userInput: string) {
        ...
        eventSource.onmessage = async (event) => {
            ...
            if (!aiMessageContainer) {
                ...
                aiMessageContainer = addMessageToUI({ author: "AI", text: "", timestamp: ""});
